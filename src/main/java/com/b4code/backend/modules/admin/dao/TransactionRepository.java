...
public interface TransactionRepository extends JpaRepository<Transaction, Long> {
  ...
  @Query("SELECT COALESCE(SUM(t.amount), 0) FROM Transaction t WHERE t.type = com.b4code.backend.modules.admin.enums.TransactionType.BOOKING_PAYMENT")
  BigDecimal sumTotalRevenue();

  @Query("SELECT COALESCE(SUM(t.amount), 0) FROM Transaction t WHERE t.type = com.b4code.backend.modules.admin.enums.TransactionType.COMMISSION")
  ...
  @Query(value = """
      ...
      """, nativeQuery = true)
  java.util.List<Object[]> getMonthlyRevenueTrend();
}
