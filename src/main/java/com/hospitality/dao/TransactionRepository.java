...
public interface TransactionRepository extends JpaRepository<Transaction, Long> {
  ...
  @Query("SELECT COALESCE(SUM(t.amount), 0) FROM Transaction t WHERE t.type = com.hospitality.enums.TransactionType.BOOKING_PAYMENT")
  BigDecimal sumTotalRevenue();

  @Query("SELECT COALESCE(SUM(t.amount), 0) FROM Transaction t WHERE t.type = com.hospitality.enums.TransactionType.COMMISSION")
