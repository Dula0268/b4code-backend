...
import com.hospitality.dto.property.SearchDTO.*;
import com.hospitality.models.GuestProperty;
...
import com.hospitality.exceptions.ResourceNotFoundException;
...
public class SearchService {
    ...
    public PaginatedResponse<PropertySearchResult> search(
        ...
        Page<GuestProperty> propertyPage = propertyRepository.searchAvailableProperties(
    ...
    // ─── GuestProperty Detail ─────────────────────────────────────────────────

    @Transactional(readOnly = true)
    public PropertyDetailResult getPropertyDetail(Long propertyId) {
        log.info("Fetching property detail for id={}", propertyId);

        GuestProperty property = propertyRepository.findById(propertyId)
                .orElseThrow(() -> new ResourceNotFoundException("GuestProperty not found: " + propertyId));

        if (!property.getPublished()) {
            throw new ResourceNotFoundException("GuestProperty not found: " + propertyId);
        }
    ...
    public FilterOptionsResponse getFilterOptions() {
        ...
        // GuestProperty Types with counts
        ...
        List<GuestProperty> allPublished = propertyRepository.findAll().stream()
                .filter(GuestProperty::getPublished)
                .collect(Collectors.toList());

        Set<String> amenitySet = new TreeSet<>();
        for (GuestProperty p : allPublished) {
    ...
    private boolean matchesAmenities(GuestProperty property, List<String> amenities) {
    ...
    private PropertySearchResult mapToPropertySearchResult(GuestProperty property, int guests) {
    ...
    private PropertyDetailResult mapToPropertyDetailResult(GuestProperty property) {
        // Gallery images
